<template>
  <div class="general-admin-resources">
    <h1>Recursos</h1>
    <b-form>
      ...
      <b-form-group label="Tipo de Recurso:" label-for="type">
        <b-form-select v-if="mode === 'save'" id="type" :options="types" v-model="resource.tipo" />
        <b-form-input v-else id="resource.tipo" type="text" v-model="resource.tipo" readonly />
      </b-form-group>
      <b-row>
        <b-col xs="12">
    ...
    <div id="table">
      <b-table striped :fields="fields" :items="getResources">
        <template v-slot:cell(actions)="data">
          ...
          <b-button variant="danger" class="btn2" @click="loadResource(data.item, 'remove')">
...
export default {
  ...
  data() {
    return {
      fields: [
        { key: "tipo", label: "Tipo", sortable: true, thClass: "text-center" },
        {
          key: "Salas",
          label: "Salas",
          sortable: true,
    ...
    loadTypes() {
      ...
      axios.get(url).then(res => {
        ...
        this.types = res.data.map(types => {
          return { ...types, value: types.idTipoDeRecursos, text: types.nome };
        });
